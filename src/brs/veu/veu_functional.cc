                                ...
                                bool scalarEnabled)
{
    ...
    switch (instruction) {
      ...
      case VeuInstruction::Compress:
        // Mikui has a decoded status bit but no VFU datapath/select signal.
        info.sourceMask = 0;
        info.supported = false;
        break;
      case VeuInstruction::MultiplyAdd:
      case VeuInstruction::MultiplySubtract:
        // Unlike Yinglong, Mikui has neither a third-source VLU path nor a
        // VMSUB VFU input. The corresponding status bits are illegal in VCU.
        info.sourceMask = 0;
        info.supported = false;
        info.rtlIllegal = true;
        break;
...
VeuFunctionalExecutor::execute(const VeuFunctionalInput &input)
{
    ...
    if (input.instruction == VeuInstruction::SlideUp ||
        ...
        // Mikui PSMU consumes operand_1[3:0] and converts it to a byte shift.
        const uint32_t shiftBytes = input.scalar & 0x0f;
        if (input.instruction == VeuInstruction::SlideUp) {
