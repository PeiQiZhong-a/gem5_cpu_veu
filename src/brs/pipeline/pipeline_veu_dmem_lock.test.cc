...
TEST(PipelineVeuDmemLockTest, DefersRvLoadUntilVeuStoreCompletes)
{
    ...
    // The cycle-aligned pipeline owns the memory transaction in EX, so place
    // the RV load in ID/EX rather than the pre-merge EX/MEM request stage.
    core.idex_cur.valid = true;
    core.idex_cur.pc = 0x80000000u;
    core.idex_cur.instr = 0x30002403u;
    core.idex_cur.kind = InstrKind::LW;
    core.idex_cur.rd = 8;
    core.idex_cur.rs1_val = 0x300u;
    core.idex_cur.imm = 0;
