...
class CompareCycleTracesTest(unittest.TestCase):
    ...
    def test_optional_veu_physical_check_separates_cpu_and_veu(self):
        physical = dict(veu_req=1, veu_addr=0x20010020, veu_we=0,
                        veu_wstrb=0, veu_wdata=0, veu_lock_start=0,
                        veu_lock_finish=0)
        rtl = load_trace(self.write(
            "rtl-veu.log", HEADER_RTL_V3,
            [cycle_v3(1, "rtl", "top_veu_regress_tb", **physical)]))
        gem5 = load_trace(self.write(
            "gem5-veu.log", HEADER_GEM5_V3,
            [cycle_v3(1, "gem5", "rtl-dut-kui-tb",
                      **{**physical, "veu_addr": 0x20010030})]))
        self.assertIsNotNone(first_veu_physical_mismatch(rtl, gem5))
        self.assertEqual(first_veu_physical_mismatch(rtl, gem5),
                         (1, ["veu_addr"]))
