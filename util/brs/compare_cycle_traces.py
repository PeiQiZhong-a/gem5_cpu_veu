...
def first_veu_physical_mismatch(
    rtl: Trace, gem5: Trace
) -> tuple[int, list[str]] | None:
    """Compare the 128-bit physical VEU SRAM pins and crossbar lock edges.

    These fields are an optional extension to brs-cycle-trace-v3, so callers
    must opt in. The CPU comparison deliberately does not imply VEU-internal
    alignment when this function has not been run.
    """
    required = {"veu_req", "veu_addr", "veu_we", "veu_wstrb",
                "veu_wdata", "veu_lock_start", "veu_lock_finish"}
    for trace in (rtl, gem5):
        for record in trace.records:
            missing = required - record.keys()
            if missing:
                raise ValueError(
                    f"{trace.path}: edge {record['edge']} lacks VEU physical "
    ...
    for edge in sorted(set(rtl_edges) | set(gem5_edges)):
        if edge not in rtl_edges or edge not in gem5_edges:
            return edge, ["missing_edge"]
        rtl_record = rtl_edges[edge]
        gem5_record = gem5_edges[edge]
        differences = [key for key in ("veu_req", "veu_lock_start",
                                      "veu_lock_finish")
                       if rtl_record[key] != gem5_record[key]]
        if rtl_record["veu_req"] == 1 or gem5_record["veu_req"] == 1:
            differences.extend(key for key in ("veu_addr", "veu_we",
                                                "veu_wstrb")
                               if rtl_record[key] != gem5_record[key])
            if rtl_record["veu_we"] == 1 or gem5_record["veu_we"] == 1:
                differences.extend(key for key in ("veu_wdata",)
                                   if rtl_record[key] != gem5_record[key])
        if differences:
            return edge, differences
    return None
